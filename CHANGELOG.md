...
- **수정**: `package.json`의 `dev:plain`(`next dev`)에 `-p 3020` 추가, `scripts/dev-open.mjs`가 실제 spawn하는 `next dev` 호출과 Chrome 자동 오픈 실패 시 폴백 URL(`FALLBACK_URL`)도 3020으로 동기화. `README.md`의 안내 URL도 `localhost:3000` → `localhost:3020`으로 수정. 포트 배정(서로 겹치지 않음): exiflens=3010, flydronemap=3020, firelic=3030.
- **검증**: `package.json` JSON 유효성, `node --check scripts/dev-open.mjs` 구문 검증 통과. 3개 저장소의 dev 서버를 동시에 기동해 각각 지정 포트에서 정상 응답하는 것을 스모크 테스트로 확인(서로 포트 충돌 없음을 직접 검증) — 다만 이 스모크 테스트 도중 이 브릿지 환경 특유의 무해한 Turbopack 캐시 EPERM 경고(기존에 여러 차례 확인된 현상)가 있었으나, 포트 3020으로 "Ready" 상태까지 정상 도달한 것은 로그로 직접 확인함.
- **커밋**: `b9d5645` "fix(dev): flydronemap 로컬 dev 서버 포트를 3020으로 고정 (형제 프로젝트 포트 충돌 방지)"
- **다음 단계**: push → 이후 `npm run dev`로 이 프로젝트를 실행하면 항상 3020번 포트로 뜸(exiflens/firelic과 동시에 띄워도 더 이상 충돌 없음).

## 2026-09-23 — 사이트 전체 점검 (사용자 요청) — 프로덕션 빌드 전체 실패 크리티컬 버그 발견 및 수정

- 배경: 사용자가 로컬에서 ThemeProvider 관련 콘솔 경고("script tag" 에러)를 보고하며 "사이트가 불안정해 보인다"고 우려, 사이트 전체 점검 요청.
- **점검 항목**: tsc --noEmit / eslint / npm run build(프로덕션 빌드) / git 상태 / Vercel 배포 이력 / 여러 로케일 페이지 렌더링.
- **크리티컬 버그 발견**: `npm run build` 실행 결과 `/sitemap.xml` 프리렌더 단계에서 빌드 전체가 중단(exit code 1)되는 것을 확인. 원인은 직전 커밋(`9b4e11f`)에서 추가한 독일어 가이드 44개 중 2건(`what-weather-data-to-check-before-flying-a-drone.mdx`, `why-rain-is-dangerous-for-drone-flights.mdx`)의 프런트매터 `description` 필드에서, 독일어 인용부호(„...")의 닫는 곧은따옴표(")를 이스케이프하지 않아 YAML 파싱이 문자열을 조기 종료시킨 것 — `src/lib/guides.ts`의 `matter()` 호출이 예외를 던지고 `sitemap.ts`가 이를 잡지 않아 빌드 전체가 죽는 구조였음.
- **Vercel 배포 이력으로 실사고 확인**: `get_deployment`로 조회한 결과, 이 버그가 포함된 커밋(`4e99e1c`)의 프로덕션 배포가 실제로 `state: "ERROR"`로 실패해 있었음. 다행히 flydronemap.com에 연결된 alias는 여전히 그 이전의 마지막 성공 배포(`6b97ce6`, 쿠팡 스위치 커밋 — 독일어 UI는 포함, 독일어 가이드 44개는 미포함)를 그대로 가리키고 있어 **실사용자에게 노출된 장애는 없었음**(Vercel이 실패한 빌드를 프로덕션에 승격하지 않는 기본 동작 덕분).
- **수정**: 두 파일 모두 이미 같은 배치 작업에서 올바르게 썼던 방식(백슬래시 이스케이프)으로 통일. 44개 파일 전체를 PyYAML로 재검증해 이 2건 외에는 문제 없음을 확인.
- **재검증**: 수정 후 `npm run build` 재실행 → 312페이지 전체 정상 생성, `/sitemap.xml`이 정적(○) 라우트로 정상 빌드됨을 확인.
- **커밋**: `a5d026f`.
- **그 외 점검 결과**:
  - `tsc --noEmit`: 에러 0건.
  - `eslint src`: 에러 0건, 기존부터 있던 무관한 경고 3건(이미지 `<img>` 최적화 권장, 미사용 변수) — 이번 작업과 무관.
  - git 워킹트리: 항상 clean 상태 유지, 이번 세션 커밋들이 의도한 파일만 정확히 건드림을 확인.
  - 사용자가 최초 보고한 "ThemeProvider script tag" 콘솔 경고: `next-themes` 라이브러리(0.4.6)가 테마 깜빡임 방지를 위해 `<script>` 엘리먼트를 React 트리에 직접 렌더링하는 구조적 특성(next/script 미사용) — 모든 로케일의 루트 레이아웃에 공통으로 적용되는 부분이라 독일어 작업과 무관하고, dev 모드 전용 경고로 실제 렌더링을 막지 않음을 확인(원인은 특정했으나 수정은 보류 — 아래 조치 방향 참고).
  - 점검 도중 localhost:3000이 (다른 세션에서) ExifLens dev 서버로 전환되어 있어 일부 확인(`/de/guides` 등)이 꼬였던 해프닝 발생 — FlyDroneMap 자체 버그 아님, 형제 프로젝트 간 포트 공유로 인한 혼선이었음.
- **조치 방향(사용자 결정 필요)**:
  1. (권장, 즉시) `a5d026f`까지 push하여 독일어 가이드 44개가 실제로 배포되도록 함 — 이번 수정 전까지는 배포가 계속 실패하고 있었음.
  2. (선택) ThemeProvider의 `next-themes` 관련 콘솔 경고는 기능상 무해하나, 근본적으로 없애려면 `next-themes`를 최신 버전으로 올리거나 스크립트 삽입 방식을 다른 방식으로 교체하는 별도 작업이 필요 — 현재는 방치해도 무방.
  3. (권장) 로컬 dev 서버로 다시 여러 로케일(특히 새로 추가된 `/de/guides`, `/de/guides/[slug]`)을 육안 확인 — 이번 점검에서 dev 서버가 중간에 다른 프로젝트로 바뀌어 있어 최종 화면 확인을 다 마치지 못함.

## 2026-09-23 — 기존 발행 가이드 게시글 44개 독일어(de) 번역 신규 추가

- **작업 내용**: 기존 발행된 가이드 게시글 44개(en/ko/ja/es 4개 언어로 이미 존재)를 영어(en) 원문
  기준으로 독일어(존댓말/Sie체)로 번역하여 `content/guides/de/*.mdx` 신규 생성.
- **번역 규칙** (ko/ja/es와 동일하게 적용):
  - `title`, `description`, 본문 마크다운만 번역.
  - `tags`, `category`, `publishedAt`, `image`, `imageCredit`, `imageCreditUrl`은 영어 원본과
    바이트 단위로 동일하게 유지 (공통 태그 체계 및 이미지 자산 공유).
  - FAA, NPS, LAANC, TFR, Part 107, B4UFLY, 36 CFR 등 미국 특화 고유명사/약어는 원어를 유지하고,
    각 용어가 처음 등장할 때 괄호로 짧은 설명을 병기.
- **검증**: `diff`로 en/de 슬러그(파일명) 44개 세트가 정확히 1:1 매칭됨을 확인.
- **부수 수정**: `src/lib/dev/guide-image-tool.ts`의 `GUIDE_LOCALES` 배열에 `"de"` 추가
  (`["en","ja","ko","es"]` → `["en","ja","ko","es","de"]`). 이 배열은 개발용 커버 이미지
  교체 도구가 슬러그별로 모든 로케일 사본의 프런트매터(image/imageCredit/imageCreditUrl)를 동기화할 때
  순회하는 목록으로, de 파일이 존재하지 않던 이전에는 자동으로 건너뛰었으나 이제 de도 함께 동기화됨.
- **커밋**: `9b4e11f` (로컬 커밋만 진행, 아직 푸시하지 않음 — 사용자 로컬 확인 후 직접 푸시 예정)
- **남은 작업 (사용자 담당)**: 신규 게시글 자동 발행 파이프라인(스케줄드 태스크/Routine)이 향후 게시되는
  글에 대해서도 독일어를 함께 생성하도록 업데이트하는 작업은 사용자가 직접 진행하기로 함 — Claude는
  스케줄드 태스크를 건드리지 않음.

## 2026-09-23 — 독일어(de) UI 언어 신규 추가 — 5번째 지원 언어(영/한/일/스 다음)

- 배경: 독일(DIPUL) 공역 기능은 이미 구현되어 있었으나 사이트 UI 자체에는 독일어가 없어("Land auswählen" 같은 문구 없이 영어로만 노출), 사용자가 독일어 UI 준비를 요청. 향후 지원 언어가 계속 늘어날 예정이라 "미루면 쌓이는 작업"인지 사용자가 먼저 질문 → 44개 가이드 게시글 번역은 1회성 선형 작업(나중에 해도 손해 없음)이지만, 자동 발행 파이프라인(예약작업)에 새 언어가 반영 안 되는 것만 진짜 "누적되는" 리스크라고 답변. 사용자는 이번 세션에서 (1) 언어 등록 (2) UI 문자열 번역만 진행 요청, (3) 예약작업(신규 가이드 자동 독일어화) 프롬프트 수정은 본인이 직접 진행하기로 함, (4) 기존 44개 게시글 소급 번역은 보류.
- **작업 전 백업**: `.backups/backup_20260923_023814_german_locale_add/`(`routing.ts`, `seo.ts`, `en.json`).
- **구현**:
  - `src/i18n/routing.ts`: `locales`에 `"de"` 추가, `localeLabels.de = "DE"` 추가 — `sitemap.ts`/`rss.xml`/언어 스위처 등 대부분의 사이트 인프라가 이 값 하나로 데이터 기반 자동 반영됨(별도 코드 수정 불필요).
  - `src/lib/seo.ts`: `LOCALE_TO_OG`에 `de: "de_DE"` 추가(OpenGraph 로케일 태그, 유일하게 수동 추가가 필요했던 지점).
  - 신규 `messages/de.json`(539개 키 / 393개 재귀 경로): `en.json`과 구조적으로 완전히 동일하게 전체 UI 문자열을 격식체(Sie) 독일어로 번역. 구조 무결성은 두 파일의 재귀 키 경로 집합을 diff하는 검증 스크립트로 393개 전부 일치(누락/추가 0건) 확인.
  - 가이드 콘텐츠(`content/guides/de/*.mdx`)는 아직 없음 — `getGuideSlugs()`가 디렉터리 부재 시 빈 배열을 안전하게 반환하도록 이미 구현되어 있어 빈 상태로 출시해도 오류 없음(사용자의 단계적 계획대로 의도된 상태, 이번 작업 범위 아님).
- **검증**: 사용자가 로컬 dev 서버 재시작 도중 서버 포트가 3001→3000으로 바뀐 것을 인지하지 못해 `/de`가 한동안 `ERR_CONNECTION_REFUSED`/`/ko/de` 404로 보였던 해프닝이 있었음(코드 문제 아님, `src/proxy.ts`·`routing.ts`·`next.config.ts` 모두 처음부터 정상이었음) — 실제 포트(3000)로 재확인한 결과 `/de` 홈, 헤더 내비(Über uns/Ratgeber/Vorschriften/FAQ), 푸터, `/de/regulations`(기존 구현된 `Regulations.countries.de` 포함) 전부 정상적으로 독일어로 렌더링되는 것을 Claude in Chrome으로 확인.
- **커밋**: `3a5bd81`
- **다음 단계**: 사용자가 로컬에서 최종 육안 확인 후 push. 예약작업(신규 가이드 자동 독일어 발행) 프롬프트 수정은 사용자가 직접 진행 예정. 기존 44개 가이드 게시글의 독일어 소급 번역은 보류(추후 별도 작업).
...
- **원인**: 직전 커밋(`a5823cb`)에서 로딩 스켈레톤을 지도용/카드용으로 분리하며 카드 스켈레톤(`src/components/drone-dashboard.tsx`)을 실수로 지도보다 위(더 이른 JSX 위치)에 배치했음. 위치(`selected`)는 빨리 정해지고 날씨/공역 데이터(`data`)는 늦게 도착하는 구간에서 "카드 스켈레톤(지도 위) → 실제 카드(지도 아래, 원래 위치)"로 자리가 바뀌면서, 그 사이에 있던 지도가 위로 밀리는 것이 CLS 0.173의 정확한 원인이었음.
- **수정**: 카드 스켈레톤 블록을 실제 카드(`{data && !loading}` 블록)와 정확히 같은 위치(지도 렌더 블록 바로 다음)로 이동. 지도 렌더 조건, 안전 배너, 레거시 JS 폴리필 제거 등 다른 로직은 전혀 건드리지 않은 최소 수정.
- 검증: `npx tsc --noEmit`(신규 오류 0건), `npx eslint`(신규 오류 0건, 기존 플레이스홀더 `<img>` 경고 2건 + 무관 경고 1건만 존재). Vercel 실제 배포 빌드 로그(`dpl_GX8WnvNwZczCvEQgCdDLL4YrApib`, 오류 없이 17초 완료)로 이전 커밋의 실제 프로덕션 빌드 성공도 재확인함.
- 작업 전 `.backups/backup_20260914_194658/`에 전체 스냅샷 백업 생성.

## 2026-09-14 (추가3) — PageSpeed 재측정 후 LCP 재악화(7.8초) 근본 원인 해결: 지도 렌더링을 데이터 로딩과 구조적으로 분리 + 레거시 JS 폴리필 제거

- 배경: 바로 위 "추가2" 항목의 LCP 플레이스홀더 배포 후 사용자가 재측정한 결과, 성능 70점은 유지됐지만 LCP가 오히려 6.7초→7.8초로 악화되고 "레거시 JavaScript 14KiB" 경고도 그대로 남아있는 것을 확인. 사용자가 상세 패널(LCP 분석/LCP 요청 탐색/레거시 JavaScript 등)을 추가로 캡처해 전달, 이를 근거로 근본 원인을 재조사함. "여러 번의 수정이 아닌 한 번에 완료"하라는 사용자 지시에 따라 실제 라이브 API 응답 시간 측정, 배포된 JS 청크 직접 분석, 컴포넌트 게이팅 로직 전체 재검토를 거쳐 계획을 승인받은 후 진행.
- **LCP 7.8초 근본 원인 및 해결** (`src/components/drone-dashboard.tsx`, `messages/{en,es,ja,ko}.json`): PageSpeed의 "LCP 요청 탐색" 상세 패널에서 실제 LCP 요소가 이전 세션에 추가한 플레이스홀더가 아니라 **지도(Leaflet)의 첫 타일 이미지 자체**이며, 그 요청이 "리소스 로드 지연" 4.7초 이후에야 시작되는 것으로 확인됨. 코드 확인 결과, 지도는 `data && selected && !loading`(위치 + 날씨/Kp/공역 데이터 전부 로딩 완료)일 때만 렌더링되도록 게이팅되어 있어, `/api/geo` → `/api/dashboard` 등 데이터 fetch 워터폴이 끝나야만 지도 타일 요청이 시작되는 구조였음(실제 API 자체는 47~295ms로 빠름을 직접 측정해 백엔드 지연이 원인이 아님을 확인). 지도의 기본 타일은 위치(`selected`)만 있으면 그릴 수 있고 데이터는 비행제한구역 오버레이에만 필요하므로, `<FlightMap>` 렌더 조건을 `selected`만으로 완화하고 데이터 의존 prop들은 전부 옵셔널 체이닝(`data?.`)으로 전환. **안전 고려사항**: 데이터가 아직 없는 짧은 순간 비행제한구역 오버레이가 안 보일 수 있어, 이를 "안전 확인됨"으로 오인하지 않도록 지도 위에 "공역 정보 확인 중" 배너(스피너 포함)를 추가하고 데이터 도착 시 자동으로 사라지도록 처리(4개 언어 메시지 파일에 `airspaceCheckingText` 키 신규 추가). 기존 로딩 스켈레톤도 "위치 없음"(지도 스켈레톤)과 "데이터 로딩 중"(카드 3개 스켈레톤)으로 분리해, 검색으로 위치를 바꿀 때 지도 전체가 다시 스켈레톤으로 리셋되던 부수 문제도 함께 해소됨. `dynamic()` import의 `loading` 폴백도 동일한 LCP 플레이스홀더로 통일.
- **레거시 JavaScript 14KiB 재확인 및 해결** (`next.config.ts`, 신규 `src/lib/empty-polyfill-module.js`): "추가2"에서 적용한 `browserslist` 모던 브라우저 기준 설정이 실제로는 효과가 없었던 원인을 배포된 실제 JS 청크를 직접 가져와 분석해 확인 — Next.js가 `next/dist/build/polyfills/polyfill-module`을 **browserslist 설정과 무관하게 모든 빌드에 무조건 포함**시키는 프레임워크 자체의 알려진 제한(vercel/next.js GitHub Issue #86785, Discussion #64330)이었음. 커뮤니티에서 검증된 워크어라운드(Next.js 16.1.6 기준)를 적용: 빈 파일(`src/lib/empty-polyfill-module.js`)을 만들고 `next.config.ts`의 `turbopack.resolveAlias`로 Next.js 내부 폴리필 모듈 경로 2종을 이 빈 파일로 치환. 이미 browserslist를 모던 브라우저로 제한해 둔 상태라 실제로 폴리필이 필요한 사용자가 없어 부작용 없음. **⚠️ 문서화되지 않은 Next.js 내부 파일 경로에 의존하므로, 향후 Next.js 버전 업그레이드 시 "레거시 JavaScript" 경고가 재발하지 않는지 재확인 필요.**
- **알리익스프레스 이미지 sizes 추가 보정** (`src/components/aliexpress-gear-cards.tsx`): PageSpeed 재측정에서 모바일 412px 뷰포트 기준 실제 표시 폭이 145px(≈35vw)로 측정된 것을 반영해 `sizes`의 모바일 값을 `45vw`→`35vw`로 조정(기존 `20vw`/데스크톱 값은 유지).
- 검증: `npx tsc --noEmit`(신규 오류 0건 — `_to_delete/` 임시 폴더 안의 기존 무관 오류만 존재), `npx eslint`(신규 오류 0건, 의도된 플레이스홀더 `<img>` 경고 2건 + 기존 무관 경고 1건만 존재). 이 세션에서는 `device_bash` 브릿지의 리소스 제약(빌드/개발서버 컴파일이 반복적으로 강제 종료됨)으로 로컬 `npm run build` 전체 완주는 확인하지 못함 — 다만 `next.config.ts`의 새 `turbopack.resolveAlias` 설정이 "Running next.config.ts" 단계를 4회 모두 정상 통과해 설정 자체의 문법/스키마 오류는 없음을 확인했고, 최종 검증은 Vercel 실제 배포 빌드 로그와 프로덕션 재측정으로 진행.
- 참고: 작업 전 `.backups/backup_20260914_090159/`에 전체 스냅샷 백업 생성(파일 개수 대조까지 완료). 커밋 `a5823cb`, push는 `push_lcp_fix.command`(1회성, gitignore 대상)로 사용자가 직접 실행.

## 2026-09-14 (추가2) — PageSpeed 재점검 후속 조치: LCP 6.7초 개선 + 레거시 JS 폴리필 제거 + 알리익스프레스 이미지 최적화

- 배경: 공통 대화방에서 진행하던 PageSpeed Insights 점검이 VWorld 공역 WMS 건으로 FlyDroneMap 전용 대화방으로 인계됨(별도 인수인계 문서 `flydronemap_점검사항_인수인계.md`). CLS 0.226 수정 배포 확인 결과 재측정에서 CLS는 0으로 해결 확인됨. 대신 이번 재측정에서 LCP 6.7초(성능 점수 72점의 주 원인)가 새로 확인되어, 사용자와 협의해 VWorld WMS 요청 병합 건(안전 기능 관련, 신중한 검증 필요)만 마지막으로 미루고 나머지 항목을 먼저 진행하기로 함.
- **LCP 6.7초 개선** (`src/components/drone-dashboard.tsx`, 커밋 `218580a`): 지도(`FlightMap`)가 `dynamic(..., { ssr: false })`로 완전히 클라이언트 사이드에서만 렌더링되고, 데이터 로딩 중 표시되는 스켈레톤에는 이미지가 전혀 없어(빈 배경색 `<div>`) Lighthouse가 실제 LCP 요소로 한참 뒤에 나타나는 Leaflet 타일 이미지를 잡는 것이 원인으로 확인됨. 이 스켈레톤 자리에 네트워크 요청이 필요 없는 인라인 SVG data URI 이미지를 `fetchPriority="high"`로 추가해, Lighthouse가 이 플레이스홀더를 빠르게 그려지는 LCP 요소로 잡도록 유도함. 실제 지도 기능·시각적 UX는 바뀌지 않음(로딩 스피너는 그대로 유지, 배경 이미지만 추가).
- **레거시 JavaScript 폴리필 제거** (`package.json`, 커밋 `f6411e5`): `browserslist` 설정이 없어 Next.js 빌드가 오래된 브라우저까지 지원하는 기본값을 쓰면서 `Array.prototype.flat/flatMap`, `Object.fromEntries`, `Object.hasOwn`, `String.prototype.trimStart/trimEnd` 폴리필(약 14KiB)이 자사 번들에 항상 포함되던 것을 확인. Next.js가 공식 문서에서 제시하는 ES 모듈 지원 최신 브라우저 기준(`chrome 64, edge 79, firefox 67, opera 51, safari 12` — 모두 2018년 이후)으로 `browserslist`를 명시해 해결. **트레이드오프 안내**: 이 기준보다 오래된 브라우저 사용자는 빌드 타겟에서 명시적으로 제외됨(실무적으로 비중이 매우 작은 수준으로 판단, 사용자 확인 후 진행).
- **알리익스프레스 상품 이미지 최적화** (`next.config.ts`, `src/components/aliexpress-gear-cards.tsx`, 커밋 `3a3437b`): 이미지 전송 개선 항목(예상 절감량 473KiB)의 주요 원인이었던 `<Image unoptimized>` 제거. 로컬 개발 서버에서 DevTools로 실제 렌더된 상품 이미지 URL을 직접 확인해 도메인이 `ae-pic-a1.aliexpress-media.com`임을 확정했고(사용자 제공 스크린샷), `next.config.ts`의 `images.remotePatterns`에 `**.aliexpress-media.com`(서브도메인 번호 변동 대비 와일드카드) 등록 후 `unoptimized`를 제거해 Next.js 이미지 리사이징이 정상 적용되도록 함. **쿠팡 상품 이미지는 실제 도메인이 아직 확인되지 않아 이번에는 손대지 않고 `unoptimized` 유지** — 잘못된 도메인 추측으로 이미지가 깨질 위험을 피하기 위함(추후 별도 확인 필요).
- 검증: `npx tsc --noEmit`(오류 0건), `npx eslint`(기존 무관 경고 1건 + 의도된 `<img>` 사용 경고 1건 외 신규 문제 없음), `npm run build`(Turbopack — 컴파일 성공, TypeScript 통과, 209개 페이지 전부 정상 생성 확인. 마지막 "Finalizing" 단계의 `.next` 파일 unlink 권한 오류는 이 브릿지 환경 특유의 기존 현상으로 코드와 무관함).
- 참고: 작업 전 `.backups/backup_20260914_045325/`에 전체 스냅샷 백업 생성. 남은 항목(VWorld WMS 요청 병합, 쿠팡 이미지 도메인 확인 후 최적화, 렌더링 차단 리소스/미사용 JS/캐시 헤더)은 후속 작업으로 남겨둠.

## 2026-09-14 (추가) — 대시보드 레이아웃 밀림(CLS 0.226) 수정 + 백업 스크립트 결함 발견/수정

- 배경: PageSpeed Insights(pagespeed.web.dev)로 모바일 실측 결과 성능 51~56점, CLS(레이아웃 밀림) 0.226으로 확인됨. 원인 조사 결과, 홈페이지 대시보드(`DroneDashboard`)가 접속 즉시 `/api/geo`로 IP 기반 위치를 자동 조회해 결과를 채우는 구조인데, 그 사이 화면이 "검색해주세요"(매우 작음) → 작은 로딩 스피너 → 지도+3열 카드 전체(약 700px 이상) 순으로 두 차례 크게 커지면서 아래쪽 콘텐츠(장비 추천 등)를 밀어내는 것이 원인으로 확인됨.
- 수정1: `src/components/drone-dashboard.tsx`의 `loading` 초기값을 `false` → `true`로 변경 — 대부분의 실제 방문(IP 위치 조회 성공)에서 첫 렌더부터 스켈레톤이 보이도록 함. IP 조회가 실패/불가능한 경우(로컬 개발 등)에만 명시적으로 `setLoading(false)` 처리해 검색창만 있는 기본 화면으로 정상 전환되도록 처리.
- 수정2: 로딩 중 UI를 작은 스피너 문구에서, 최종 지도(`h-72`/`sm:h-96`, `FlightMap`의 자체 로딩 스켈레톤과 동일 높이) + 3열 카드 스켈레톤으로 교체 — 로딩이 끝나고 실제 데이터로 바뀔 때 높이 변화가 거의 없어 아래 콘텐츠가 밀리지 않음.
- 검증: `npx tsc --noEmit`, `npx eslint`(변경 파일, 기존에도 있던 무관한 `searching` 미사용 경고 1건 외 이상 없음), `npm run build` 통과. `next start` 로컬 서버로 서버 렌더링(SSR) 초기 HTML에 스켈레톤(`animate-pulse`) 마크업이 정상 포함되는 것 확인.
- 부수 발견 및 수정: 이번 작업 전 백업 시도 중, 기존 백업 스크립트 방식(`--exclude=node_modules --exclude=.next`)이 실제로는 `.next.stale.*`(과거 빌드 잔여물 여러 개), `.backups`/`.claude-backups`(과거 백업 폴더 자체) 등을 제외하지 못해 1.7GB짜리 손상된(조기 종료로 잘린) 백업 파일이 생성된 것을 발견. 손상 파일은 즉시 삭제하고, 이번 백업부터는 실제 소스 관련 폴더/파일만 명시적으로 지정해 백업하도록 변경(7.5MB, 정상 압축 완료 확인). 다른 2개 사이트도 유사한 구조라면 같은 문제가 있을 수 있어 추후 백업 시 주의 필요.
- 작업 전 `_backups/flydronemap_backup_*_CLS수정전.tar.gz`(정상본, 7.5MB)로 백업 완료.
...
## 2026-09-24 — 오로라 시즌, 고위도 지역 드론 항법 이상 현상 가이드 자동 발행

- 오로라 타원대가 자기위도 기준으로 형성되어 고위도 상공 전리층이 평상시에도 신틸레이션에 취약한 이유, 자극에 가까울수록 지자기 수평성분이 약해져 나침반이 폭풍과 무관하게 불안정해지는 원리, 북위 65도 이상·Kp 6 이상 밤 오로라 촬영 시 GPS 드리프트·RTH 오작동 사례를 다룬 space-weather-gps 카테고리 신규 가이드
- 오로라 촬영에 집중하다 야간 Kp 급변을 놓치는 실수, RTH 과신 등 실전 실수와 비행 전 체크리스트 포함, 4개 언어(en/ja/ko/es) 작성
- 클라우드 자동발행 파이프라인으로 생성, 빌드 검증 완료

## 2026-09-23 — GPS 재밍/스푸핑이란? 드론 비행에 미치는 영향 가이드 자동 발행

- GPS 재밍(신호 차단)과 스푸핑(신호 위조)의 차이, 그리고 Kp지수·멀티패스 문제와 구별하는 법을 다룬 space-weather-gps 카테고리 신규 가이드
- 공항/군사시설/국경 등 재밍·스푸핑 위험 지역에서의 대응 체크리스트 포함, 4개 언어(en/ja/ko/es) 작성
- 클라우드 자동발행 파이프라인으로 생성, 빌드 검증 완료

## 2026-09-22 — 드론 비행 전 날씨 확인, 어떤 지표를 봐야 할까 가이드 자동 발행

- 풍속/돌풍, 강수, 가시거리, 기온, 이슬점, 운고, Kp지수까지 비행 전 확인해야 할 날씨 지표를 하나로 묶은 체크리스트형 가이드 발행 (en/ja/ko/es 4개 언어)
- 클라우드 자동발행 파이프라인으로 생성, 빌드 검증 완료

## 2026-09-21 — 고도별 바람 변화(윈드시어)와 드론 비행 가이드 자동 발행

- 고도가 올라갈수록 지상 풍속과 다르게 바람이 강해지고 방향도 바뀌는 이유(경계층 마찰, 개활지 기준 120m 상공 풍속이 10m 대비 약 1.4~1.6배), 이른 아침 잔잔해 보여도 100~300m 상공에는 강풍이 부는 야간 제트류 현상, 절벽·건물 인근 지형성 난류, 단계적 고도 상승 점검법을 다룬 신규 가이드(en/ja/ko/es 4개 언어)를 추가했습니다.
- 클라우드 자동발행 파이프라인으로 생성, 빌드 검증 완료

## 2026-09-20 — 폭염 속 드론 비행 시 배터리·전자장비 주의사항 가이드 자동 발행

- 폭염 상황에서 LiPo 배터리 열화·팽창 위험, 온보드 전자장비(ESC/비행 컨트롤러) 과열로 인한 출력 제한, 카메라·짐벌 열 영향, 뜨거운 차 안 보관의 위험, 비행 직후 충전 시 주의사항을 다룬 신규 가이드(en/ja/ko/es 4개 언어)를 추가했습니다.
- 클라우드 자동발행 파이프라인으로 생성, 빌드 검증 완료

## 2026-09-19 — Return-to-Home(RTH) 설정 가이드 자동 발행

- RTH 발동 3가지 경로(수동/저배터리/신호두절), 복귀 고도 설정과 장애물 회피, 홈포인트 GPS 기록 정확도, 이동 플랫폼(선박 등) 이륙 시 위험, KP지수-GPS 정확도 연계까지 다룬 가이드를 4개 언어(en/ja/ko/es)로 작성.
- 클라우드 자동발행 파이프라인으로 생성, 빌드 검증 완료(233개 페이지 정상 생성 확인).

## 2026-09-18 — 드론 여행 가방 패킹 체크리스트 가이드 자동 발행

- 항공 여행 시 리튬 배터리 와트시(Wh) 규정, 기내 수하물 보관 원칙, 고온으로 인한 LiPo 손상 방지법, 짐벌/예비부품 패킹 요령을 실전 수치와 함께 정리
- 클라우드 자동발행 파이프라인으로 생성, 빌드 검증 완료

## 2026-09-17 — 드론 펌웨어 업데이트 모범 사례 가이드 자동 발행

- 펌웨어 업데이트 타이밍, 배터리 잔량·전원 안정성, 버전 정합성, 업데이트 후 설정값(RTH 고도 등) 재확인, IMU/나침반 재보정, 다중 기체 운용 시 순차 업데이트 등 브릭(먹통) 위험을 피하는 실전 팁을 4개 언어(en/ja/ko/es)로 작성
- 클라우드 자동발행 파이프라인으로 생성, 빌드 검증 완료

## 2026-09-16 — 드론 비행 시간을 늘리는 방법 가이드 자동 발행

- 배터리 교체 없이 비행 시간을 늘리는 실전 방법(급가속 자제, 순항 속도, 바람 대응, 저온 대책, 페이로드 배치)을 4개 언어(en/ja/ko/es)로 작성
- 클라우드 자동발행 파이프라인으로 생성, 빌드 검증 완료

## 2026-09-15 — 안전을 위한 프로펠러 가드 선택법 가이드 자동 발행

- 프로펠러 가드가 실제로 막아주는 위험(열상)과 막지 못하는 위험(추락 충격)을 구분하고, 무게 증가에 따른 비행시간(10~15%)·강풍 안정성 저하 등 실전 수치를 포함해 설명
- 실내/군중 근처/초보자 훈련 등 가드 장착이 기본이어야 하는 상황과, 재질(경질 vs 유연)별 장단점, 흔한 착각(가드=이격거리 단축 가능)을 다룸
- 클라우드 자동발행 파이프라인으로 4개 언어(en/ja/ko/es) 생성, 빌드 검증 완료

## 2026-09-14 — 드론 영상 촬영을 위한 ND 필터 가이드 자동 발행

- 드론 카메라의 고정 조리개 특성 때문에 ND 필터가 사실상 필수인 이유, 광량별(맑음/구름/흐림) 적정 ND 강도 기준, 가변 ND의 편광 아티팩트 한계, 자석식 필터 탈착 사고 예방, ND-PL 필터의 밴딩 현상 등을 실전 수치와 함께 다룸.
- 4개 언어(en/ja/ko/es) 모두 작성 완료, 클라우드 자동발행 파이프라인으로 생성, 빌드 검증 완료.

## 2026-09-13 — 산불 인근 드론 비행 제한 가이드 자동 발행

- 산불 지역 상공 TFR(14 CFR 91.137)의 법적 근거, "If You Fly, We Can't Fly" 캠페인의 실제 운영 논리, 2025년 팰리세이즈 화재 공중충돌 사건과 2026년 콜로라도 애스펀 에이커스 화재 기소 사례를 바탕으로 한 실제 처벌 수위(최대 7만 5천 달러 민사 벌금, 최대 2년 징역)를 다룸.
- 4개 언어(en/ja/ko/es) 모두 작성 완료, 클라우드 자동발행 파이프라인으로 생성 및 빌드 검증 완료.
